...
### Khi lệnh không chạy được trong môi trường agent
- Nếu một lệnh quan trọng bị lỗi do sandbox, quyền hệ điều hành, npm/cache, network, GUI, execution policy, timeout bất thường, hoặc output không đủ rõ để kết luận, agent không được cố đoán là pass.
- Sau tối đa 1-2 lần thử hợp lý trong môi trường agent, phải dừng vòng lặp và đưa nguyên lệnh ngắn gọn để user tự chạy bên ngoài terminal.
- Riêng `dart format`: agent **không tự chạy lệnh này** trong môi trường Codex vì thường bị treo/timeout rất lâu trên Windows; nếu cần format Dart, báo rõ cho user để user tự chạy bên ngoài terminal.
- Khi đưa lệnh cho user, phải ghi rõ thư mục cần `cd`, mục đích của lệnh, và đoạn output cần copy lại nếu lệnh vẫn fail.
- Không được xem các lỗi môi trường kiểu `EPERM`, `EACCES`, PowerShell execution policy, npm cache, hoặc CDK/Lambda bundling timeout là lỗi code nếu chưa có bằng chứng từ output.
...
### Đồng bộ component và navigation
- Các component cùng vai trò phải dùng chung pattern, spacing, màu sắc, icon, state, và interaction. Không được tự tiện tạo một biến thể visual mới chỉ vì đang làm ở màn khác.
- Đặc biệt với bottom navigation, bottom bar, tab bar, action bar, và các control cố định ở đáy màn hình: nếu màn hình đã có bottom component tương đương trong repo thì phải reuse hoặc bám sát component/pattern hiện có.
- Không được để tình trạng màn A dùng một bottom bar, màn B cũng là bottom bar nhưng tự tạo layout/màu/behavior khác mà không có yêu cầu rõ ràng.
- Chỉ được tạo bottom/component biến thể mới khi user yêu cầu trực tiếp, hoặc khi có khác biệt product/architecture bắt buộc; khi đó phải ghi rõ lý do trong plan hoặc comment ngắn gọn gần code liên quan.
- Khi sửa một component shared hoặc một pattern đã xuất hiện nhiều nơi, phải kiểm tra các màn hình liên quan để tránh lệch UX giữa các route.
