...
void main() {
  ...
  testWidgets(
    'shows one icon placeholder when no gallery thumbnails are available',
    (tester) async {
      await tester.pumpWidget(buildButton(const <Uint8List>[]));

      expect(
        find.byKey(const ValueKey('gallery-preview-placeholder')),
        findsOneWidget,
      );
      expect(find.byIcon(Icons.photo_library_outlined), findsOneWidget);
      expect(
        find.byKey(const ValueKey('gallery-preview-back-photo')),
        findsNothing,
      );
      expect(
        find.byKey(const ValueKey('gallery-preview-front-photo')),
        findsNothing,
      );
    },
  );
  ...
  testWidgets(
    'shows only the back thumbnail when exactly one gallery photo exists',
    (tester) async {
