...
describe('create-checkin handler audience', () => {
  ...
  it('creates an all-friends check-in when audience.type is ALL_FRIENDS', async () => {
    mockQuery
      .mockResolvedValueOnce({ rows: [{ id: 'checkin-1', created_at: '2026-06-12T01:00:00.000Z' }] })
    ...
    expect(mockQuery).toHaveBeenNthCalledWith(
      ...
      expect.arrayContaining(['user-1', 'place-1', null, 'media-1', 10.7738, 106.7035, null, null, null, 'FRIENDS', 'ALL_FRIENDS']),
    );
  ...
  it('creates direct recipient rows for multiple selected friends when audience.type is DIRECT', async () => {
    mockQuery
      ...
      .mockResolvedValueOnce({ rows: [{ id: 'checkin-2', created_at: '2026-06-12T01:01:00.000Z' }] })
      .mockResolvedValueOnce({ rows: [] })
      .mockResolvedValueOnce({ rows: [] })
      .mockResolvedValueOnce({ rows: [] });
